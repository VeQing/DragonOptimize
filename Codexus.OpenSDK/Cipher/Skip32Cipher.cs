namespace Codexus.OpenSDK.Cipher;

public static class Skip32Cipher
{
    private static readonly byte[] FTable =
    [
        
    ];

    private static int G(byte[] key, int k, int w)
    {
        var g1 = w >> 8;
        var g2 = w & 0xff;

        var g3 = FTable[g2 ^ (key[4 * k % 10] & 0xFF)] ^ g1;
        var g4 = FTable[g3 ^ (key[(4 * k + 1) % 10] & 0xFF)] ^ g2;
        var g5 = FTable[g4 ^ (key[(4 * k + 2) % 10] & 0xFF)] ^ g3;
        var g6 = FTable[g5 ^ (key[(4 * k + 3) % 10] & 0xFF)] ^ g4;

        return (g5 << 8) + g6;
    }

    private static void Skip32(byte[] key, int[] buf, bool encrypt)
    {
        int k;
        int step;

        if (encrypt)
        {
            step = 1;
            k = 0;
        }
        else
        {
            step = -1;
            k = 23;
        }

        var wl = (buf[0] << 8) + buf[1];
        var wr = (buf[2] << 8) + buf[3];

        var i = 0;
        while (i < 24 / 2)
        {
            wr ^= G(key, k, wl) ^ k;
            k += step;
            wl ^= G(key, k, wr) ^ k;
            k += step;
            ++i;
        }

        buf[0] = wr >> 8;
        buf[1] = wr & 0xFF;
        buf[2] = wl >> 8;
        buf[3] = wl & 0xFF;
    }

    public static uint Encrypt(uint value, byte[] key)
    {
        return (uint)Encrypt((int)value, key);
    }

    public static int Encrypt(int value, byte[] key)
    {
        var buf = new int[4];
        buf[0] = (value >> 24) & 0xff;
        buf[1] = (value >> 16) & 0xff;
        buf[2] = (value >> 8) & 0xff;
        buf[3] = value & 0xff;

        Skip32(key, buf, true);

        var output = (buf[0] << 24) | (buf[1] << 16) | (buf[2] << 8) | buf[3];
        return output;
    }

    public static uint Decrypt(uint value, byte[] key)
    {
        return (uint)Decrypt((int)value, key);
    }

    public static int Decrypt(int value, byte[] key)
    {
        var buf = new int[4];

        buf[0] = (value >> 24) & 0xff;
        buf[1] = (value >> 16) & 0xff;
        buf[2] = (value >> 8) & 0xff;
        buf[3] = value & 0xff;

        Skip32(key, buf, false);

        var output = (buf[0] << 24) | (buf[1] << 16) | (buf[2] << 8) | buf[3];

        return output;
    }
}